...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const key = `chat:${ip}`;
  if (!allowRequest(key)) {
    return NextResponse.json({ message: '요청이 너무 잦습니다. 잠시 후 다시 시도하세요.', retryAfterMs: 60_000, remaining: 0 }, { status: 429 });
  }
  ...
  if (process.env.MOCK_AI_RESPONSES === '1') {
    const mockVerses = relevantVerses.length > 0 
      ? relevantVerses.map(v => ({ book: v.book, chapter: v.chapter.toString(), verse: v.verse.toString() }))
    ...
    await ChatRecord.create({ sessionId, role: 'assistant', content: mock.content, verses: mock.verses, prayer: mock.prayer });
    return NextResponse.json(mock);
  }

  const openai = new OpenAI({ apiKey });
  try {
    const completion = await withCircuitBreaker(() => retryWithBackoff(async () => {
...
기도는 간단하고 실용적으로 제시하세요.${versesContext}`,
    ...
    // 중복 제거
    const uniqueVerses = combinedVerses.filter((verse, index, self) => 
    ...
    await ChatRecord.create({ sessionId, role: 'assistant', content: payload.content, verses: payload.verses, prayer: payload.prayer });
