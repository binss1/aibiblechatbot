...
// 간단한 다단계 상담 구현
async function handleMultiStepCounseling(openai: OpenAI, model: string, sessionId: string, message: string) {
  ...
  if (state.step === 'initial') {
    ...
    if (updatedState.questionCount >= 4) {
      ...
      return await generateFinalResponse(openai, model, sessionId, updatedState);
    } else {
      // 다음 질문 제시
    ...
    // 후속 상담
    return await handleFollowupStep(openai, model, sessionId, message, state);
  }
}

// 종합 분석 및 최종 응답 생성
async function generateFinalResponse(openai: OpenAI, model: string, sessionId: string, state: any) {
  const allContent = [state.initialConcern, ...state.answers].join(' ');
...
기도는 간단하고 실용적으로 제시하세요.${versesContext}`,
                ...
                {
                  role: 'user',
                  content: `초기 고민: ${state.initialConcern}\n\n상세 답변들:\n${state.answers.map((answer: string, index: number) => `${index + 1}. ${answer}`).join('\n')}`,
                },
              ],
            },
